...
class DQN(Agent):
    ...
    def init(self):
        ...
        if self.update_target != 0:
            ...
            if self.doubel_dqn_tuned:
                self.log("Using the tuned version of Double DQN (shared action layer)")
                self.online_policy.l1 = self.policy.l1
        else:
            self.online_policy = self.policy

        if self.recurrent:
            self.log(f"Using DRQN with hidden_dim={self.hidden_dim}")
...
class DDQN(DQNHardUpdate):
    epsilon: HYPERPARAM = Hyperparam(start=0.9, end=0.01)
