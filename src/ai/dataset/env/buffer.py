...
class ReplayBuffer(BaseMP):
    ...
    def model_post_init(self, __context):
        ...
        self.reset()
        self._setup()
        self.info(f"Setup buffer of size {self.capacity}")
    ...
    def setup_delayed(self, delayed_info: dict[str, DelayedInfo]):
        ...
        self.info("Setting up delayed keys", delayed_info)
    ...
    def memorize(self, memo: dict):
        ...
        for k, v in memo.items():
            if k in self._delayed_info:
                self.warn_once(f"Ignoring key {k} since it is delayed (not stored)")
                continue
