...
from ..modality import Modality, NormalizedModality
from .bbox import bboxes


class ChannelData(NormalizedModality):
    channel_first: Consumed[Optional[bool]] = None

    bounding_boxes: bboxes = []

    @classmethod
    def modality_format(cls, tensor: torch.Tensor, kwargs: dict[str, Any]) -> Self:
        if tensor.ndim == 2:
            tensor = tensor[None, :, :]
        elif tensor.ndim == 3:
            channel_first = kwargs.get("channel_first", None)
            if channel_first is not None and not channel_first:
                tensor = tensor.permute(1, 2, 0)

        return tensor

    def to_channel_last(self) -> Self:
        return self.rearrange("c h w -> h w c")

    def show(self):
        from PIL import Image as PImage

        PImage.fromarray(self.to_channel_last().safe().bitmap()).show()

    def bitmap(self) -> Modality:
    ...
    #     self.log_msg(f"Using augmentations:\n{augs}")
    ...
    #         cls.log_msg(f"Saved image to {path}")
    ...
    def _transforms_from_conf(self, conf: Union[dict, str, list, int, float, tuple]):
        args, kwargs = [], {}
        try:
    ...
    def _preprocessor(self):
        if "preprocess" not in self.image:
            self.log_msg("Using default resize to 256x256")
